...
def init_state():
    """
    This function initializes and returns the state dictionary with the following keys:
    - changed: boolean indicating if the state has changed
    - network: boolean indicating if the network is present
    - step: integer indicating the step count
...
def run_simulation_test():
    """
    This function runs a simulation test for the server network. It sets up a server network instance with the given parameters, such as number of servers, maximum processes, routing policy, and load balancing algorithm.
    The function also calculates the arrival probability for each time step based on the context information, such as day of the week, hour of the day, and other factors, using a weight matrix and a normal distribution function.
    The simulation runs for a certain number of time steps and for each step, the function generates a request, if applicable, and handles it by calling the handleRequest() method of the server network. The function also updates the state of the network after each step.
    At the end of the simulation, the function updates the network one last time and saves the final state history to the 'network' key in the state dictionary.
    ...
    serverNetwork = ServerNetwork(round(config['max_servers']/2), config['max_processes'], config=config,
...
def run_simulation_live():
    """
    This function runs a simulation test for the server network. It sets up a server network instance with 
    the given parameters, such as number of servers, maximum processes, routing policy, and load balancing algorithm.
    The function also calculates the arrival probability for each time step based on the context information, such as 
    day of the week, hour of the day, and other factors, using a weight matrix and a normal distribution function.
    The simulation runs for a certain number of time steps and for each step, the function generates a request, 
    if applicable, and handles it by calling the handleRequest() method of the server network. The function also 
    updates the state of the network after each step.
    Just before the start of each period the function waits for the user to select the number of users to use for the period.
    At the end of the simulation, the function updates the network one last time and saves the final state history to the 
    'network' key in the state dictionary.
...
def build_profit_table():
    """
    Builds a table of network profit information, including the hour, number of servers used, 
    rewards, server costs, fail costs, and profit for each hour.

    Returns:
        list: A list containing a HTML string for the total profit and a Dash DataTable for the profit table.
    """
    build_data = []
...
def build_arrival_graph():
    ...
    x = list(range(0, len(state['network']['arrivals']['small'][:-1])))
    fig.add_bar(x=x, y=state['network']['arrivals']['small'][:-1], name="Small requests")
    fig.add_bar(x=x, y=state['network']['arrivals']['large'][:-1], name="Large requests")
    fig.update_layout(barmode="relative")
    fig.update_xaxes(
        range=[-0.7, len(state['network']['arrivals']['small'][:-1])-0.5])
...
def build_completion_graph():
    completed_small = [r / config['reward_small'] for r in state['network']['profit']['rewards_small']]
    completed_large = [r / config['reward_large'] for r in state['network']['profit']['rewards_large']]
...
def build_server_profit_graph():
    ...
    fig.update_xaxes(range=[0, len(state['network']['servers_used'])])
    fig.add_hline
    return dcc.Graph(
            ...
            html.Div(
                ...
                children=[
                    ...
                    daq.LEDDisplay(
                        id="time-led",
                        value=(
                            '0' if config['start_time'] < 10 else '') + str(config['start_time']) + ":00",
                        color="#92e0d3",
...
def update_live_arrivals_graph(n):
    if len(state['network']['arrivals']['small']) > 1:
        arrivals = state['network']['arrivals']['small'][-2] + \
            state['network']['arrivals']['large'][-2]
        x = round(state['step'] / config['steps'], 2)
        arrivals = arrivals if arrivals > 0 else state['network']['arrivals']['small'][-3] + \
            state['network']['arrivals']['large'][-3]
...
def render_network_tab(test_btn, live_btn, mode_btn, test_start_btn, sim_run_finished_btn):
    ...
    match button_id:
        ...
        case 'mode-change-btn':
            return 'start-screen',
        case 'sim-run-finished-btn':
